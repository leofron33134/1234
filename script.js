...
// Math.round(audio.volume)


const initialWidth = 100;
const initialHeight = 100;
const step = 50;
const volumeStepStandart = 0.5; // 50%
...
resetButton.addEventListener('click', function() {
  ...
  p.innerText = audio.volume
  audio.src = "./sounds/zvuk.mp3"
});
...
soundPlus.addEventListener('click', function(){
  if (audio.volume < 1) {
        ...
        // p.innerText = Math.round(audio.volume, -1)
        p.innerText = audio.volume.toFixed(1)
});
...
switchSound.addEventListener('click', function() {
  console.log(audio.src);
  const switchSrc = audio.src;

  if (currentSrc.includes("./sounds/zvuk.mp3")) {

    audio.src = './sounds/zvukMonkey.mp3';
  } else {
    console.log('1234');
    audio.src = "./sounds/zvuk.mp3";
  }
